...
def parse_tunnel_graph():
    parser = argparse.ArgumentParser(
        description='evaluate throughput and delay of a log and '
        'generate graphs')
...
class TunnelGraph(object):
    ...
    def parse_tunnel_log(self):
        ...
        if capacities:
            ...
            for bin_id in list(range(min(capacity_bins), max(capacity_bins) + 1)):
        ...
        for flow_id in self.flows:
            ...
            if flow_id in arrivals:
                ...
                for bin_id in list(range(min(ingress_bins), max(ingress_bins) + 1)):
            ...
            if flow_id in departures:
                ...
                for bin_id in list(range(min(egress_bins), max(egress_bins) + 1)):
